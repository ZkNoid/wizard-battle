...
export class TournamentLeaf extends Struct({
  ...
  hash(): Field {
    const prizeFields = (this.prizePercents as UInt32[]).flatMap((p) =>
      p.toFields()
    );
...
export class TournamentManager extends SmartContract {
  ...
  @method async createTournament(
    ...
    config.feePercent.assertLessThanOrEqual(
      UInt32.from(5000),
      'Fee cannot exceed 50%'
    );
    config.claimWindow.assertGreaterThan(
      UInt32.from(0),
      'Claim window must be > 0'
    );
    ...
    const [rootBefore, key] = tournamentWitness.computeRootAndKey(Field(0)) as [
      Field,
      Field
    ];
  ...
  @method async sponsorFund(
    ...
    const newSponsorContribution =
      currentTournament.sponsorContribution.add(amount);
  ...
  @method async claimPrize(
    ...
    const newPrizePool = currentTournament.prizePool.sub(
      currentWinnerLeaf.prizeAmount
    );
