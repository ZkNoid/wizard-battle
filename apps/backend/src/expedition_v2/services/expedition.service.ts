...
import type { ExpeditionTimePeriod, IExpeditionRewardDB } from '@wizard-battle/common';
...
export class ExpeditionService {
   ...
   */
  async getUserExpedition(userId: string, expeditionId: string): Promise<Expedition> {
    const expedition = await this.expeditionModel
      .findOne({ id: expeditionId, userId })
      .exec();
    if (!expedition) {
      throw new NotFoundException(`Expedition with id "${expeditionId}" not found`);
    }
    return expedition;
  }
   ...
   */
  async completeExpedition(userId: string, expeditionId: string): Promise<Expedition> {
   ...
   */
  async interruptExpedition(userId: string, expeditionId: string): Promise<Expedition> {
    const expedition = await this.getUserExpedition(userId, expeditionId);
  ...
  async getActiveExpeditionCount(userId: string): Promise<number> {
    return this.expeditionModel.countDocuments({ userId, status: 'active' }).exec();
  }
}
