...
export class RedisHealthService {
    ...
    constructor() {
        ...
        this.redisClient.connect().then(() => console.log('RedisHealthService Redis Connected'));
    }
    ...
    async getDetailedStats() {
        try {
            ...
            const instanceGroups = mappings.reduce((acc, mapping) => {
        ...
        try {
            ...
            for (const [socketId, mappingStr] of Object.entries(socketMappings)) {
                try {
                    ...
                    if (mapping.roomId && !activeRooms.has(mapping.roomId) && !activeGameStates.has(mapping.roomId)) {
            ...
            console.log(`Cleanup completed: ${cleaned} items cleaned, ${errors} errors`);
        } catch (error) {
