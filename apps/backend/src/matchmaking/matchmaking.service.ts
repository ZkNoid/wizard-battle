...
export class MatchmakingService {
     ...
     * The service immediately connects to Redis and starts the 20-second matchmaking cycle.
     */
    ...
    private async processMatchmaking() {
        try {
            ...
            if (waitingPlayers.length < 2) {
                console.log(`Not enough players for matching. Current queue size: ${waitingPlayers.length}`);
                return;
            }

            console.log(`Processing matchmaking for ${waitingPlayers.length} players`);
            ...
            if (activeMatches && Object.keys(activeMatches).length > 0) {
                for (const [_, matchData] of Object.entries(activeMatches)) {
                    try {
                        ...
                        if (match.player1?.playerId) activePlayerIds.add(match.player1.playerId);
                        if (match.player2?.playerId) activePlayerIds.add(match.player2.playerId);
                    } catch (error) {
            ...
            const filteredQueuedPlayers = queuedPlayers.filter(qp => 
                qp.player.playerId && !activePlayerIds.has(qp.player.playerId)
            );
            ...
            const staleEntries = queuedPlayers.filter(qp => qp.timestamp < fiveMinutesAgo);
            
            if (staleEntries.length > 0) {
                console.log(`Cleaning up ${staleEntries.length} stale queue entries`);
                for (const staleEntry of staleEntries) {
                    try {
                        await this.redisClient.lRem('waiting:queue', 1, JSON.stringify(staleEntry));
                    } catch (error) {
            ...
            if (filteredQueuedPlayers.length < 2) {
                console.log(`Not enough available players for matching after filtering. Available: ${filteredQueuedPlayers.length}`);
                return;
            }

            console.log(`Processing matchmaking for ${filteredQueuedPlayers.length} available players (${queuedPlayers.length - filteredQueuedPlayers.length} already in matches)`);
            console.log(`Actual Redis queue length: ${await this.redisClient.lLen('waiting:queue')}`);
 ...
 * - Queue and matches stay synchronized
     ...
     */
    private async createMatch(player1: IPublicState, player2: IPublicState): Promise<void> {
        if (!player1.playerId || !player2.playerId) {
            console.error('Cannot create match: missing player IDs');
            return;
        }

        console.log(`Creating match between ${player1.playerId} and ${player2.playerId}`);

        // Sort player IDs for consistent room ID
        const [firstPlayer, secondPlayer] =
            player1.playerId < player2.playerId ? [player1, player2] : [player2, player1];
        ...
        if (existingMatch) {
            console.warn(`Match already exists for room ${roomId}, skipping creation`);
            return;
        }
        ...
        for (const entry of waitingPlayers) {
            try {
                ...
                if (queuedPlayer.player.playerId === firstPlayer.playerId || queuedPlayer.player.playerId === secondPlayer.playerId) {
        ...
        if (removedCount !== 2) {
            console.warn(`Expected to remove 2 players, but only removed ${removedCount}`);
        }

        console.log(`Successfully created match between ${firstPlayer.playerId} and ${secondPlayer.playerId} in room ${roomId}`);
    }
     ...
     */
    private async notifyPlayersOfMatch(player1: IPublicState, player2: IPublicState, roomId: string): Promise<void> {
        if (!this.server) {
        ...
        if (socket1) {
            ...
            const opponentSetup1: IPublicState = new TransformedPlayerSetup(
                ...
                `Wizard${player2.playerId!}`,
                100,
                player2.mapStructure!,
                player2.spells!,
                player2.level!
            );
            matchFound1 = new TransformedFoundMatch(
                roomId,
                player2.playerId!,
                [opponentSetup1]
            );
        ...
        if (socket2) {
            ...
            const opponentSetup2: IPublicState = new TransformedPlayerSetup(
                ...
                `Wizard${player1.playerId!}`,
                100,
                player1.mapStructure!,
                player1.spells!,
                player2.level!
            );
            matchFound2 = new TransformedFoundMatch(
                roomId,
                player1.playerId!,
                [opponentSetup2]
            );
    ...
    async joinMatchmaking(socket: Socket, addToQueue: IAddToQueue) {
        ...
        const addToQueueResponse: IAddToQueueResponse = new TransformedAddToQueueResponse(
            true, 
    ...
    async leaveMatchmaking(socket: Socket) {
        ...
        const removeFromQueue: IRemoveFromQueue = new TransformedRemoveFromQueue(socket.id, 0, null);
