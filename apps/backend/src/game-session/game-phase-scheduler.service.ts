...
export class GamePhaseSchedulerService {
  ...
  async cleanupInactiveRooms() {
    try {
      ...
      const inactiveRooms = await this.gameStateService.getInactiveRooms(1800000); // 30 minutes
  ...
  private async getPendingPhaseTransitions(): Promise<PhaseTransition[]> {
    ...
    const roomKeys = await this.gameStateService.redisClient.keys('game_state:*');
   ...
   */
  private async executePhaseTransition(transition: PhaseTransition): Promise<void> {
    const { roomId, currentPhase, nextPhase } = transition;
    
    try {
      console.log(`🔄 Phase transition: ${roomId} ${currentPhase} → ${nextPhase}`);
  ...
  private async getSystemHealth() {
    ...
    for (const key of roomKeys.slice(0, 100)) { // Sample for performance
      const roomId = key.replace('game_state:', '');
      const gameState = await this.gameStateService.getGameState(roomId);
