...
import { MageBotStrategy } from './strategies/mage-bot.strategy';
 ...
 *
 * BREAKING CHANGE (refactor: bot strategy pattern):
 *   `BotService` has been removed. All wizard selection, spell picking, action
 *   generation and trusted-state simulation logic now lives in `IBotStrategy`
 *   implementations under `bot/strategies/`.
 *
 *   Before:  new BotClient(botId, url, new BotService())
 *   After:   new BotClient(botId, url, new MageBotStrategy())   // or Archer / PhantomDuelist
 *
 *   `createBotClient()` accepts an optional third `strategy` argument
 *   (default: `MageBotStrategy`). Pass a different strategy when matchmaking
 *   allows the user to choose a bot opponent type.
 */
@Injectable()
export class BotClientService {
   ...
   * @param strategy AI strategy that controls wizard identity and decisions;
   *                 defaults to MageBotStrategy
  ...
  async createBotClient(
    ...
    strategy: IBotStrategy = new MageBotStrategy()
  ): Promise<BotClient> {
    console.log('[DEBUG] Bot client connecting to:', serverUrl);
    const bot = new BotClient(botId, serverUrl, strategy);
    await bot.connect();
 ...
 * @dev Handles all game events and delegates AI decisions to the provided IBotStrategy
 */
export class BotClient {
  ...
  constructor(
    ...
    private readonly strategy: IBotStrategy
  ) {
    // Initialize bot with default state generated by the strategy
    this.currentState = this.strategy.generateSetup(botId, '');
  }
  ...
  private submitActions(): void {
    ...
    try {
      const actions = this.strategy.generateActions(
        this.botId,
        this.currentState,
  ...
  private submitTrustedState(): void {
    ...
    // Delegate state computation to the strategy
