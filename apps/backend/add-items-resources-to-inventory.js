...
async function addWoodToInventory() {
  try {
    ...
    const userAddress = args[0] || process.env.USER_PUBLIC_KEY;
    ...
    if (!userAddress) {
      ...
      console.log('Or set USER_PUBLIC_KEY in .env file');
      process.exit(1);
    }
...
async function addLegendarySwordToInventory() {
  try {
    // Get command line arguments
    const args = process.argv.slice(2);
    const userAddress = args[0] || process.env.USER_PUBLIC_KEY;
    ...
    if (!userAddress) {
      console.error('❌ Error: User address is required');
      console.log(
        'Usage: node add-legendary-sword-to-inventory.js <userAddress>'
      );
      console.log(
        'Example: node add-legendary-sword-to-inventory.js 0x70997970C51812dc3A010C7d01b50e0d17dc79C8'
      );
      console.log('Or set USER_PUBLIC_KEY in .env file');
      process.exit(1);
    }
    ...
    // Find LegendarySword resource
    const legendarySwordResource = await GameItem.findOne({
      name: 'LegendarySword',
    });
    if (!legendarySwordResource) {
      console.error('❌ Error: LegendarySword resource not found in database');
      console.log(
        '💡 Tip: Run "node add-item-resource.js" first to add LegendarySword resource'
      );
      await mongoose.connection.close();
      process.exit(1);
    }

    console.log('📦 Found LegendarySword resource:');
    console.log('   ID:', legendarySwordResource._id);
    console.log('   Name:', legendarySwordResource.name);
    console.log('   Rarity:', legendarySwordResource.rarity);
    console.log('\n👤 User Address:', userAddress);
    console.log('📊 Quantity to add:', quantity, '\n');

    // Check if user already has LegendarySword
    const existingInventory = await UserInventory.findOne({
      userId: userAddress,
      itemId: legendarySwordResource._id,
    });
    ...
    if (existingInventory) {
      ...
      const savedInventory = await newInventory.save();
      console.log('✅ Added LegendarySword to user inventory:');
      console.log('   Inventory ID:', savedInventory._id);
      console.log('   Quantity:', savedInventory.quantity);
      console.log('   Acquired From:', savedInventory.acquiredFrom);
    }
...
(async () => {
  await addWoodToInventory();
  await addLegendarySwordToInventory();
})();
