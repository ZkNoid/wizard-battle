...
class HumanVsHumanTester {
  ...
  async runTest() {
    ...
    console.log(`📊 Target: ${this.maxRounds} rounds between two human players\n`);
  ...
  async setupPlayers() {
    ...
    await Promise.all(this.players.map(player => new Promise((resolve, reject) => {
      ...
      setTimeout(() => reject(new Error(`${player.name} connection timeout`)), 10000);
    })));
  ...
  setupEventListeners() {
    this.players.forEach((player, playerIndex) => {
      ...
      player.socket.on('newTurn', (data) => {
        if (playerIndex === 0) { // Only count once per round
          this.currentRound++;
        ...
        }, 1000 + (playerIndex * 500)); // Stagger submissions slightly
      });

      // Phase 2: Spell Propagation
      player.socket.on('allPlayerActions', (allActions) => {
        if (playerIndex === 0) { // Only count once per phase
          this.testResults.phasesCompleted++;
        }
        
        console.log(`📋 ${player.name} received actions from ${Object.keys(allActions).length} players`);
      ...
      player.socket.on('applySpellEffects', () => {
        ...
        }, 2000 + (playerIndex * 500)); // Stagger submissions
      });

      // Phase 5: State Update
      player.socket.on('updateUserStates', (data) => {
        const roundDuration = Date.now() - this.roundStartTime;
        console.log(`🔄 ${player.name} received state update (Round ${this.currentRound} - ${roundDuration}ms)`);
      ...
      player.socket.on('gameEnd', (data) => {
        console.log(`🏆 ${player.name} received game end - Winner: ${data.winner || 'Draw'}`);
        this.endTest();
      });
    ...
    const trustedState = {
      ...
      publicState: {
        ...
        fields: [
          ...
          ...Array(117).fill(0).map(() => new Field(Math.floor(Math.random() * 50)))
        ]
      },
  ...
  generateReport() {
    ...
    console.log(`🎯 Rounds Completed: ${this.testResults.roundsCompleted}/${this.maxRounds}`);
    console.log(`🔄 Total Phases Completed: ${this.testResults.phasesCompleted}`);
    console.log(`⚠️  Synchronization Errors: ${this.testResults.synchronizationErrors}`);
    
    console.log('\n📊 PLAYER STATISTICS:');
    console.log(`👤 Player 1 - Actions: ${this.testResults.playerStats.player1.actionsSubmitted}, States: ${this.testResults.playerStats.player1.statesSubmitted}`);
    console.log(`👤 Player 2 - Actions: ${this.testResults.playerStats.player2.actionsSubmitted}, States: ${this.testResults.playerStats.player2.statesSubmitted}`);
    ...
    if (this.testResults.synchronizationErrors === 0) {
      ...
      console.log(`⚠️  WARNING: ${this.testResults.synchronizationErrors} synchronization issues`);
    }
    
    const expectedActions = this.testResults.roundsCompleted * 2; // 2 players per round
    const actualActions = this.testResults.playerStats.player1.actionsSubmitted + 
