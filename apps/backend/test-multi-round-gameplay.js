...
class MultiRoundGameplayTester {
  ...
  setupEventListeners() {
    ...
    this.playerSocket.on('allPlayerActions', (allActions) => {
      ...
      console.log(
        `📋 Phase 2: SPELL_PROPAGATION - Received actions from ${Object.keys(allActions).length} players`
      );
      ...
      if (botActions) {
        ...
        console.log(
          `   🤖 Bot participated (Total bot actions: ${this.testResults.playerStats.bot.actionsSubmitted})`
        );
      }
    });
    ...
    this.playerSocket.on('updateUserStates', (data) => {
      ...
      console.log(
        `🔄 Phase 5: STATE_UPDATE - Round ${this.currentRound} completed (${roundDuration}ms)`
      );
  ...
  submitRoundActions() {
    ...
    console.log(
      `⚡ Phase 1: SPELL_CASTING - Submitting ${actions.actions[0].spellId}`
    );
  ...
  async testWinnerLoserScenarios() {
    ...
    console.log('✅ Winner/Loser scenarios testing completed\n');
  }
    ...
    return new Promise((resolve) => {
      ...
      this.playerSocket.on('gameEnd', (data) => {
        if (!gameEndReceived) {
          ...
          console.log(`   ✅ Winner detected: ${data.winnerId}`);
          console.log(`   📊 Expected winner: ${this.matchData.playerId}`);
      ...
      // Kill the bot
      console.log('   💀 Killing bot player...');
      this.playerSocket.emit('reportDead', {
        roomId: this.matchData.roomId,
  ...
  async testSelfDeath() {
    console.log('🎯 Test 2: Self-Death Scenario');
    ...
    return new Promise((resolve) => {
      ...
      this.playerSocket.on('gameEnd', (data) => {
        if (!gameEndReceived) {
          ...
          console.log(
            `   📊 Expected winner: ${this.matchData.opponentId} (bot should win)`
          );

          if (data.winnerId === this.matchData.opponentId) {
            console.log('   ✅ PASS: Bot correctly wins when human dies');
          } else if (data.winnerId === 'draw') {
            console.log('   ⚠️  UNEXPECTED: Draw when only human died');
          } else {
            console.log('   ❌ FAIL: Incorrect winner for self-death');
          }
      ...
      console.log('   💀 Human player committing suicide...');
      this.playerSocket.emit('reportDead', {
        roomId: this.matchData.roomId,
      ...
      setTimeout(() => {
        if (!gameEndReceived) {
          console.log('   ❌ TIMEOUT: No game end event for self-death');
          resolve();
        }
      }, 5000);
    });
  ...
  async testSimultaneousDeath() {
    console.log('🎯 Test 3: Simultaneous Death (Draw) Scenario');
    ...
    return new Promise((resolve) => {
      ...
      const handleGameEnd = (data) => {
        ...
        console.log(
          `   📊 Game end #${gameEndCount}: Winner = ${data.winnerId}`
        );
        ...
        if (gameEndCount >= 1) {
          ...
          if (lastWinner === 'draw') {
            console.log(
              '   ✅ PASS: Draw correctly detected for simultaneous death'
            );
          } else {
            console.log(
              `   ⚠️  RESULT: Winner declared as ${lastWinner} instead of draw`
            );
            console.log(
              '   📝 NOTE: This may be expected if deaths are processed sequentially'
            );
          }
          resolve();
        }
      ...
      setTimeout(() => {
        if (gameEndCount === 0) {
          console.log(
            '   ❌ TIMEOUT: No game end event for simultaneous death'
          );
          resolve();
        }
      }, 5000);
  ...
  async testInvalidDeathReport() {
    console.log('🎯 Test 4: Invalid Death Report Handling');
    ...
    return new Promise((resolve) => {
      ...
      this.playerSocket.on('gameEnd', (data) => {
        ...
        console.log(
          `   ⚠️  UNEXPECTED: Game end triggered by invalid death report`
        );
      });
      ...
      setTimeout(() => {
        console.log('   🔍 Testing null death report...');
        this.playerSocket.emit('reportDead', {
          roomId: this.matchData.roomId,
          dead: { playerId: null },
        });
      ...
      setTimeout(() => {
        if (!unexpectedGameEnd) {
          console.log('   ✅ PASS: Invalid death reports correctly ignored');
        } else {
          console.log('   ❌ FAIL: Invalid death report triggered game end');
        }
        resolve();
      }, 4000);
    });
  ...
  generateReport() {
    ...
    console.log(
      `🎯 Rounds Completed: ${this.testResults.totalRounds}/${this.maxRounds}`
    );
    console.log(
      `⏱️  Total Test Duration: ${Math.round(this.testResults.performanceMetrics.totalTestDuration / 1000)}s`
    );
    console.log(
      `📈 Average Round Duration: ${Math.round(this.testResults.performanceMetrics.averageRoundDuration)}ms`
    );
    console.log(`🔄 Completed Phases: ${this.testResults.completedPhases}`);

    console.log('\n📊 PLAYER STATISTICS:');
    console.log(
      `👤 Human - Actions: ${this.testResults.playerStats.human.actionsSubmitted}, States: ${this.testResults.playerStats.human.statesSubmitted}`
    );
    console.log(
      `🤖 Bot - Actions: ${this.testResults.playerStats.bot.actionsSubmitted}, States: ${this.testResults.playerStats.bot.statesSubmitted}`
    );
    ...
    if (this.testResults.performanceMetrics.memoryLeaks.length > 1) {
      ...
      if (memGrowth < 50) {
        console.log(
          `✅ PASS: Memory growth under control (${memGrowth.toFixed(1)}%)`
        );
      } else {
        console.log(
          `⚠️  WARNING: Significant memory growth (${memGrowth.toFixed(1)}%)`
        );
      }
    }
